...
def default_cost_function(num_cells, resolution):
  ...
  return 1e10000 if num_cells > pow(_GEOCELL_GRID_SIZE, 2) else 0

def best_bbox_search_cells(bbox, cost_function = default_cost_function):
...
def parents(cell):
  return [cell[:i] for i in range(1, len(cell))]
